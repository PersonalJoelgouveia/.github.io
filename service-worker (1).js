 ...
 * Estratégia:
 *  - Network First → App Shell (index.html/manifest), pra sempre pegar
 *    atualizações publicadas quando houver sinal; cai pro cache só offline.
 *  - Cache First   → Google Fonts (CSS + arquivos .woff2), que não mudam.
 ...
 * e nunca ficar preso numa versão antiga quando há sinal disponível.
 */
const SW_VERSION = 'vf-v2';
...
self.addEventListener('fetch', (event) => {
  ...
  // 3) Mesma origem (app shell: HTML/CSS/JS embutido no index.html) →
  // Network First. Era Cache First antes — o que fazia o app SEMPRE
  // servir a versão antiga em cache, mesmo com uma nova publicada no
  // GitHub Pages, e só buscar na rede se o cache estivesse vazio. Com
  // sinal (o caso normal), busca sempre a versão mais nova; sem sinal
  // (academia sem wifi), cai pro último snapshot salvo — mantém o
  // objetivo original de funcionar offline.
